...
document.getElementById("logout").addEventListener("click",function() {
    showModalLogout("🫡 Are you sure you want to leave this page ?", "ATTENTION !");
});
...
window.onpopstate = function () {
    history.pushState(null, "", location.replace);
    showModalNotice("😏 You cannot go back.","ATTENTION !");
}
...
function redact (e){
    ...
    const andro = ["Sunday", "Monday", "Thuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
    const volana = [
    ...
    // format heure anglais Start Time
    let [hour, minute] = sTimeStr.split(":").map(Number);
    ...
    if (minute === 0) {
        resultStime = `${hour12} o'clock ${ampm}`;
    } else if (minute === 15) {
        resultStime = `quarter past ${hour12} ${ampm}`;
    } else if (minute === 30) {
        resultStime = `half past ${hour12} ${ampm}`;
    } else if (minute === 45) {
        resultStime = `quarter to ${nextHour12} ${ampm}`;
    } else if (minute < 30) {
        resultStime = `${minute} minutes past ${hour12} ${ampm}`;
    } else {
        resultStime = `${60 - minute} minutes to ${nextHour12} ${ampm}`;
    }

    // format heure anglais end Time
    let [hourEnd, minuteEnd] = eTimeStr.split(":").map(Number);
    const ampmEnd = hourEnd >= 12 ? "PM" : "AM";
    
    // 24h to 12h conversion
    const hour12End = hourEnd % 12 === 0 ? 12 : hourEnd % 12;
    const nextHour12End = (hour12End % 12) + 1;
    
    let resultetime = "";
    
    if (minuteEnd === 0) {
        resultetime = `${hour12End} o'clock ${ampmEnd}`;
    } else if (minuteEnd === 15) {
        resultetime = `quarter past ${hour12End} ${ampmEnd}`;
    } else if (minuteEnd === 30) {
        resultetime = `half past ${hour12End} ${ampmEnd}`;
    } else if (minuteEnd === 45) {
        resultetime = `quarter to ${hour12End} ${ampmEnd}`;
    } else if (minuteEnd < 30) {
        resultetime = `${minuteEnd} minutes past ${hour12End} ${ampmEnd}`;
    } else {
        resultetime = `${60 - minuteEnd} minutes to ${nextHour12End} ${ampmEnd}`;
    }
    
    // condition si formulaire vide
    if (!nbOne || !nbTwo || !inpPlace || !dateStr || !sTimeStr || !eTimeStr || !lead || !nbrParticipant || !inpAgenda || !inpObjective || checkedValuesTwo.length === 0 || checkedValues.length === 0 ){
        if (!inpPlace || !dateStr || !sTimeStr || !eTimeStr || !inpAgenda || !inpObjective){
            showModalNotice("😡 Please fill in all the fields !","ATTENTION !");
        } else if (checkboxes.length === 0 ){
            showModalNotice("😏 Who among the board members were present ?","ATTENTION !");
        } else if (!nbrParticipant){
            showModalNotice("😏 How many were present ?","ATTENTION !");
        } else if (!nbOne || !nbTwo){
            showModalNotice("😏 What is the number of this report ?","ATTENTION !");
        } else if (checkedValuesTwo.length === 0 ){
            showModalNotice("😏 Who served as the secretary ?","ATTENTION !");
        }
        ...
        const formData ={
            ...
            outPlace : inpPlace,
            outDate : formated,
            outStime : resultStime,
            outEtime: resultetime,
            outLead: lead,
        ...
        setTimeout (()=> {
            window.location.href ="../output/eng/outeng.html";
        }, 12000);
    }
